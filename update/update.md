...
- 刷入自动禁用coolapk@[嘟嘟ski](coolmarket://u/734809)大佬的**4线程Dex2oat**(`dex2oat_4t`)模块，本模块和该模块目的及参数一致，不冲突，但重复。
#### **32.0**
- 尝试修复部分设备上的`inotify`问题。
#### **31.9**
- 调整**增量更新**的一些逻辑。
#### **31.8**
- 添加Compressor(`compress.joshattic.us`)，奇妙视频(`com.magicalstory.videos`)，锤子音乐(revived)(`app.smartisanmusic.revived`)适配。
- 调整**光遇**FMOD音频线程。
#### **31.7**
- 修正一个`core_architect_set`导致的除`4+3+1`以外的模块分配错误的bug，强烈建议更新。
#### **31.6**
- 添加漫读(`com.mangareader`)适配。
- 添加崩坏:因缘精灵(`com.miHoYo.abccbtest`)，遗忘之海(`com.netease.ywzh`)适配。
- 基于`1.6.3`的正式版移除`60秒`过滤机制。
- 修复一个`dumpsys cpuinfo`的bug，感谢[Github @ZYB14](https://github.com/ZYB14)反馈。
#### **31.5**
- 添加Joiplay新旧版本的`Ren'py`插件线程，阅读plus(`io.legado.app.releaseS`)，PiliPlusX (`com.gucooing.piliplus`)，醒图(Hypic)(`com.xt.retouchoversea`)适配。
- 修正`core_architect_set`的一个bug，`update_config`和`modtify_config`flag文件**必须搭配使用**。
#### **31.4**
...
#### **31.0**
- 更新**Roblox/超自然行动组**线程。
- 添加适配敢达:争锋对决 (`com.tencent.tmgp.gundambattle`,`com.kingnet.gundam.mi`,`com.kingnet.gundam.aligames`)，奇葩战斗家 (`com.leiting.daluandou.aligames`,`com.leiting.daluandou.mi`,`com.leiting.daluandou.vivo`,`com.leiting.daluandou.huawei`,`com.leiting.daluandou.nearme.gamecenter`)，麻雀一番街 (`com.riichicity.happywoods`)，星之翼(Starward) (`com.xzy.xgst`,`com.xzy.styx`)，地平线行者(HorizonWalker) (`com.GentleManiac.HorizonWalker`,`com.GentleManiac.HorizonWalkerGoogle`)，现代战舰 (`com.zhanhuo.mw`,`com.zhanhuo.mw.aligames`,`com.zhanhuo.mw.mi`,`com.Shooter.ModernWarships`,`com.tencent.tmgp.mw`)。
...
#### **30.7**
- 添加ONScripter Plus(`com.onscripter.plus`)适配。
- 添加瘟疫公司(`com.miniclip.plagueinc`)、植物大战僵尸:冰河世纪(`com.popcap.pvz2mmhd`)适配。
- `get_cpu_core_Info`函数改成以**CPU序号**排序，避免`2+3+2+1`有的人分不清。
#### **30.6**
